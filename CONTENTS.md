...
# Part Zero — Getting the book working

Before any of it applies, a reader has to open the box.

0. Opening the box

# Part One — Agreeing on words

The first conversation in a new chain, before anything else. **Language only.** The full
staff used to sit here; they have moved to Part Two, where they are hired into a building
that already exists.
...
You cannot decide how a place is run until you know what is in it. This part is the
inventory: everything the building contains, sorted into categories rather than listed
item by item.

**The seam with Part Three:** Part Two says a thing *exists*. Part Three says how it is
*used*. Part Two says there is a counter where a finished dish is checked and a taster who
never cooked it. Part Three says how the tasting is done. Every chapter in Part Three
should point back at something Part Two put in the building.

Each chapter is one category, and each category carries a test for what belongs in it.

3. **The building** — anything fixed to the ground; if you moved premises you would leave it
...
Detail agreed while furnishing the building, parked against the chapter it belongs to.

**Ch. 9, the people — the starting roster.** The book ships a default set of roles, not a
blank form; a kitchen renames them, but does not start from nothing.

...
**What this chapter settles:** why the doorman enforces a rule without words, and what the restaurant must never replace with a reminder.
...
That makes it the clearest example of a rule that is not words.
...
The service chapters begin with the restaurant already standing and ready. That can only be
true if the doorman is already working. **If the guard cannot refuse the shared main kitchen,
the restaurant is not ready for service.** A request to use a separate bench is not enough,
because a request may be forgotten or ignored.

This chapter settles what the working guard must enforce. Chapter 49 settles what must come
in the box to make that protection work. The two jobs must not be confused. Naming the guard
does not make it active, and naming its means here would turn this chapter into a list of
tools.
...
1. Treat the doorman as an automatic guard, never as a written instruction or a staff role.
2. Have the doorman allow work only at the proper separate bench and refuse work in the
   shared main kitchen.
3. Keep the doorman separate from the taster. Have it protect separation, never judge a
   dish.

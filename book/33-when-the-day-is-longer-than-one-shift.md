...
**What this chapter settles:** how a second shift takes over a continuing day without losing the restaurant's memory or weakening its checks.

## A longer day spends one shift's attention

A pile of waiting work is not the signal to add a shift. The strain arrives when the day
lasts longer than one set of workers can keep the restaurant in view. **Their attention runs
out while the restaurant still needs to continue.**

The answer is another shift wearing the same seven base roles. It is not a new role, and it
does not create a shift manager. The floor remains jobs, not bodies, with three pairs of
hands.

## The handover is written before the shift is added
...
The operations log remains one diary. **Never start another log for the incoming shift and
never rewrite what the outgoing shift recorded.** Each open order remains where its last
stamp left it.

The incoming shift reads the end of the log, finds each last stamp and takes the next open
job from the writing. The outgoing shift adds what actually happened before leaving. The
incoming shift adds what happens next. Neither shift depends on the other's memory.
...
The owner receives a decision, never the unfinished work of running the shift.

## Never turn handover into closing and opening

Starting a fresh log or repeating the whole day's close can feel tidy. Do not do it. It cuts
one continuing day into separate memories and hides where an open order was left.
...
1. Add another shift only when the day outlasts one shift's attention. Never use a pile of

...
| ~~**T-05**~~ | ~~Chapter 44 states its governing rule **twice in eight lines** ("blank form, not a finished answer" / "blank forms, not answers").~~ **CLOSED 2026-07-23.** Chef `cook-t05` drove Codex via `model-flow mf-b4fada628765` to remove the duplicate 3-line restatement, keeping the clearer instance; Codex reviewer APPROVED. Head chef verified the rule now appears once. | done | **Rule stated once; taster SERVE 5/5 zero faults; sweep all 11 NONE; body 490 ≤700 — measured PASS.** | "Chapter 44 states its governing rule twice in eight lines" (closed) |
...
| # | Ticket | Priority | Done when | Evidence |
|---|---|---|---|---|
| ~~**T-11**~~ | ~~The same two brief files still say **"chef"** (worker sense) — the book says *cook*.~~ **CLOSED 2026-07-23.** Chef `cook-t11` (GLM-tier, own worktree) fixed 4 worker-sense "chef"/"chefs" → "cook"/"cooks" across both brief files; the "head chef" role left intact. Head chef verified independently. | done | **grep-clean: only "head chef" remains in both files; both parse as JSON — measured PASS.** | "The press shop's own scripts still say the chefs are AI agents" (residual, closed) |
...
| ~~**T-17**~~ | ~~The roster names who does each job and is **silent on who covers** an unreachable chair.~~ **CLOSED 2026-07-23.** Chef `cook-t17` drove Codex (`model-flow mf-9881a27cd207`) to add rule 9 "No check without separation" to `THE-PRINT-SHOP.md` — a chair is covered only where none of the three separations breaks (printer odd-jobs OK; taster never covered by the page's writer; doctor never folded into a worker it diagnoses; else *no checker, no service*). The review cycle caught and fixed a real slip (doctor checks fitness, not page content) before APPROVED. | done | `THE-PRINT-SHOP.md` now states coverage, tied explicitly to the three separations; addition-only (10 lines, no roster row altered) — head chef verified. | "The odd-jobs chair was unreachable and the work went to the printer" (closed) |
| ~~**T-13**~~ | ~~`press/prep.py` still holds a **fourth hand-kept copy** of the brief's six field names.~~ **CLOSED 2026-07-23.** Chef `cook-t13` drove Codex (`model-flow mf-41e0a3a0f486`, through a fix cycle that also caught a second duplicate in the rule bullets) to build the prompt's field shape by `zip`-pairing `brief_contract.BRIEF_LIST_FIELDS`; the reviewer APPROVED. | done | **Rename test: renaming a field in `brief_contract.py` changed prep's output with no prep edit; byte-identical prompt before/after the refactor; head chef confirmed `brief_contract.py` unchanged and only the `validate_brief` field literal remains — measured PASS.** | "A fourth copy of the brief's field names survives" (closed) |
